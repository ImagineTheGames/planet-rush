/**
 * tests/sim/resource-fairness.test.ts — equal resources for every station
 ...
 * player count), so:
 *
 *   1. every home station's LOCAL ore (within radius R of it) is equal — and,
...
/** The station ring radius (all stations equidistant from the centre). */
function ringRadius(w: World): number {
  const c = center(w);
  return dist(w.stations[0]!.pos, c);
}
...
describe('every home station has the same local ore — exactly (field rule §1)', () => {
    ...
    for (const seed of SEEDS) {
      for (const n of COUNTS) {
        ...
        for (const p of w.stations) {
          ...
          expect(local, `seed ${seed} × ${n}: station ${p.id} local rocks`).toHaveLength(
            RESOURCE_FIELD.homeCount,
          );
          expect(
            local.every((a) => a.home === p.owner),
            `seed ${seed} × ${n}: station ${p.id} local rocks are all its own home field`,
          ).toBe(true);
        }
      }
    }
    ...
    for (const seed of SEEDS) {
      for (const n of COUNTS) {
        ...
        for (const p of w.stations) {
          ...
          // Positions are a rigid rotation → distances-to-own-station match to
  ...
  it('per-player distance profile to their k nearest ore rocks is identical', () => {
    for (const seed of SEEDS) {
      for (const n of COUNTS) {
        ...
        const profiles = w.stations.map((p) =>
          w.asteroids
        ...
        for (let pi = 1; pi < profiles.length; pi++) {
          for (let i = 0; i < k; i++) {
            expect(
              profiles[pi]![i],
              `seed ${seed} × ${n}: station ${pi} nearest-${i} distance`,
...
describe('the fair field respects the world rules (field rule §4)', () => {
  it('keeps every home field outside its station turret range', () => {
    // GDD balance: a home field must not sit under the guns of its own station,
      ...
      for (const n of COUNTS) {
        ...
        for (const p of w.stations) {
          for (const a of w.asteroids) {
            ...
            expect(
              dist(a.pos, p.pos),
              `seed ${seed} × ${n}: station ${p.id} home rock inside turret range`,
  ...
  it('keeps a clear launch pocket around every home — no rock inside SPAWN_CLEAR_POCKET', () => {
    ...
    // so a ship leaving its station bumped rock (the mobile drag test caught it). The
    // pocket is the fix — NO asteroid body may intrude within `SPAWN_CLEAR_POCKET`
    // of ANY station. Checked on the full field (home fields AND the whole commons)
    // and for every seed and lobby size: because the pocket is part of the rotated
    // per-station stamp it is identical for everyone, so it holds universally.
    for (const seed of SEEDS) {
      for (const n of COUNTS) {
        const w = fullField(seed, n);
        ...
        for (const p of w.stations) {
          for (const a of w.asteroids) {
            ...
            expect(
              ...
              `seed ${seed} × ${n}: rock ${a.id} intrudes into station ${p.id}'s spawn pocket`,
  ...
  it('the pocket is generous — the launch lane from spawn to the first rock is wide', () => {
    // "Generous, not minimal" (brief): a ship spawns orbiting its station, so the
    // pocket must clear well past the spawn point. Assert the pocket comfortably
    // exceeds the spawn's offset from the station, i.e. the ship itself sits inside
    // the clear pocket with room to move before the field begins.
    for (const seed of [1, 42, 20260725]) {
      for (const n of COUNTS) {
        const w = fullField(seed, n);
        const pocket = pocketRadius(w);
        // Ship spawn radius from its station — the ship rings its station inboard by
        // `STATION.orbitOffset`, so this is that gap, read straight off the world.
        for (const s of w.ships) {
          const home = w.stations.find((p) => p.owner === s.id)!;
          const spawnToStation = dist(s.home, home.pos);
          expect(
            pocket,
            `seed ${seed} × ${n}: pocket must clear past ship ${s.id}'s spawn`,
          ).toBeGreaterThan(spawnToStation);
        }
      }
    }
  });

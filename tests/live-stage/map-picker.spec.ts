/**
 * tests/live-stage/map-picker.spec.ts — the MAP PICKER is wired into boot, and the
 * arena you pick is the arena the sim builds. OWNER: UI Engineer (GDD §2.1; map
 * registry m8-01; picker m8-02).
 *
 * The picker's failure mode is the same dark-matter class the M2 HUD and the M4
 * lobby were retracted for: a screen that is model-green (its cards, its layout,
 * its selection all unit-tested in `src/ui/map-picker.test.ts`) but never actually
 ...
 * bundle, picking a map, and reading the booted world's own planets can.
 *
 ...
 * read-only `window.__mapPicker` / `__mainMenu` / `__lobby` seams `main.ts`
 * installs:
 *
 *   MAIN MENU (4 map cards, octagon selected, diamond VETERAN) → pick a map →
 *   PLAY → LOBBY → RUSH! → the booted world's home planets ARE that map's registry
 *   layout, exactly.
 *
 * The last step is the one that matters: `worldPlanets` is read back off the
 * *actual* sim world `bootOfflineMatch` built, and compared to `expected[mapId]` —
...
// for thumbs"). `hasTouch` lays the picker out at thumb scale.
test.use({
...
/** Open a CLEAN boot on the main menu and wait for the picker seam to install. */
async function openMenu(page: Page): Promise<void> {
  await page.goto('/', { waitUntil: 'load' });
  await page.waitForSelector('canvas', { state: 'attached', timeout: 30_000 });
  await page.waitForFunction(() => typeof window.__mapPicker?.selectMap === 'function', undefined, {
    timeout: 20_000,
  });
}

test('the picker shows four cards — octagon selected, diamond VETERAN — thumb-sized in landscape', async ({
  page,
}) => {
  ...
  expect(p.visible, 'the picker is on screen on the menu').toBe(true);
  expect(p.screen).toBe('menu');
  // Four cards — the four ratified maps (GDD §2.1; registry m8-01).
  expect(p.cardCount, 'four map cards').toBe(4);
  ...
  expect(p.selectedId, 'octagon preselected the first time out').toBe('octagon');
  expect(p.defaultId).toBe('octagon');
  expect(p.veteranId).toBe('diamond');
  // Thumb-sized and wholly inside the safe (landscape) viewport.
  expect(p.isTouch, 'a phone lays the picker out at thumb scale').toBe(true);
  ...
  // Evidence for the PR body — the picker as a first-time player meets it.
  await page.screenshot({ path: 'tests/live-stage/map-picker-evidence.png' });

  expect(pageErrors, 'no page errors on the menu').toEqual([]);
});

test('picking each map then PLAY boots that arena — the world planets ARE the registry layout', async ({
  page,
}) => {
  const pageErrors: string[] = [];
  page.on('pageerror', (e) => pageErrors.push(String(e)));
  ...
  await openMenu(page);
  const order = await page.evaluate(() => window.__mapPicker!.order);
  ...
  for (const mapId of order) {
    ...
    await page.evaluate((i) => window.__mapPicker!.selectMap(i), index);
    const selected = await page.evaluate(() => window.__mapPicker!.selectedId);
    expect(selected, `picking card ${index} selects ${mapId}`).toBe(mapId);

    // PLAY → lobby → RUSH! → the world boots.
    ...
    // Wait for the built world's planets to be reported onto the picker seam.
    await page.waitForFunction(() => window.__mapPicker?.worldPlanets != null, undefined, {
      timeout: 20_000,
    });

    const built = await page.evaluate(() => ({
      worldMapId: window.__mapPicker!.worldMapId,
      worldPlanets: window.__mapPicker!.worldPlanets,
      expected: window.__mapPicker!.expected,
    }));
    ...
    // …and every home planet sits exactly where the registry places it (m8-02:
  ...
  expect(pageErrors, 'no page errors across four map boots').toEqual([]);
});
...
test('the last arena picked survives a reload (localStorage, same seam as the fire mode)', async ({
  ...
  await page.evaluate(
    (id) => window.__mapPicker!.selectMap(window.__mapPicker!.order.indexOf(id)),
    target,
  );
  expect(await page.evaluate(() => window.__mapPicker!.selectedId)).toBe(target);

  // Reload — the pick is read back from storage and the same card is preselected.
  await openMenu(page);
  expect(
    await page.evaluate(() => window.__mapPicker!.selectedId),

...
**What a wedge says, and what it still may not *(amended 2026-08-06 — the ratified Gantry/Bone build wheel, `docs/design/gantry-bone-handoff.md`; executed by u7-02)*.** The developer ratified the design's build-wheel screen, and it settles two things this section previously left the wheel poorer for:

- **The cost reads `cost/held`** — `3/4`, `5/4`: what the thing costs over what the player can actually spend. The second half is not new information (it is the same live total already sitting in the hub), it is that total **restated at the point of decision**, which is what lets the wheel drop any "you need 2 more" copy — the numbers already say it. An unaffordable cost is coloured; the colour rule and its limits are `style-guide.md` §2.1.
- **Every capped wedge shows its count over its cap** — `2 / 4 BUILT`, `0 / 2 BUILT`, `0 / 1 BUILT`. Previously only the radar satellite did this, and turret and shield capped *silently*: the wedge greyed out and the player was left guessing whether the ring was full or the bank was empty. Two per-station caps are design rules (above), so they are worth showing; and because **the count includes queued construction**, the number a player reads is exactly the number the cap is enforced on. It is also the re-arm tell — when a turret is shot down the count drops and the wedge lights again.

**Neither is a number on a segment, and that distinction is load-bearing.** Both ship as *labels*: `cost/held` is one string, not two numbers, and the count is a string too. The rule this section states — a segment's only number is its cost — is what stops a rate, a DPS, or a stat leaking onto the wheel, and it is enforced structurally (`src/ui/build-wheel.test.ts` asserts a segment's numeric fields are exactly its angle and its cost). The wheel gained two lines of copy and no new number. **UPGRADE SHIP still carries its arrow rather than a price**, and the wheel is still the only place a player buys anything.

**The upgrade wheel says it the same way *(amended 2026-08-06 — same ratified direction, executed by u7-06)*.** The screen behind UPGRADE SHIP is the same radial control, so it now speaks the same grammar rather than a second one a player has to learn between two presses:

- **Its cost reads `cost/held` too** — `12/8`. Exactly the change above, for exactly the reason above: this wheel dims a track the player cannot afford, and before this the reason lived in the hub at the other end of the screen. `12/8` says it where the decision is made. A finished ladder quotes **`MAX`** where a capped build wedge quotes `FULL` — no price, because there is nothing left to buy.

...
### 2.1 Match setup *(amended 2026-07-27 — modes, slot model, variable size, four maps; amended 2026-08-05 — sides read FRIENDLY / ENEMY + letter; **amended 2026-08-07 — an `open` slot is EMPTY: `N` counts humans plus bots, and RUSH! is refused below two participants**; **amended 2026-08-07 — the host picks each bot's CHARACTER, and its difficulty is shown**; **amended 2026-08-07 — ship select and map select are SCREENS opened from the lobby, which keeps one card of each (§2.11)**; see `docs/design-amendments.md` and `docs/variable-slots-plan.md`)*

**Two modes and a slot model (amended).** A match runs in one of two modes, chosen in the lobby: **Free-for-All** (every station is its own side) or **Teams** (facilities grouped by side, alliances made explicit). Under the hood these are one model — FFA is simply "teams-of-one," so the whole simulation asks "are these two enemies?" through a single predicate and Teams changes a table, not a code path. The lobby shows **8 physical slots**, each set to **open** (a competitive human seat), **bot** (AI-filled), or **closed** (excluded from the match entirely — no ship, no station, no color). A newly created room seeds every slot but the host's **open**, and nothing is AI-filled until the host says so *(amended 2026-08-07)*.

**What `N` counts (amended 2026-08-07).** The earlier rule — *"N is the count of non-closed slots, 2–8"* — assumed an open slot always became a bot, so it counted the same thing either way. It no longer does. **`N` is the count of humans plus bots, 2–8.** An `open` slot nobody took contributes nothing: no ship, no station, no color, no bot — it is simply not in the match, exactly as a `closed` one is not, and the only difference between the two is that a joiner can still take an open one. Closing slots is still how a host shapes a 3-station duel or a 2v2; it is now the way to say *"and stop anyone else arriving"* rather than the only way to shrink the match. **RUSH! is refused, with a reason on screen, below two participants** — a host alone in a room they have put no bots in has no match to start, and a button that launched a one-player war would be the lie the count exists to prevent. In Teams the host assigns each slot a side; team sizes may be uneven (a 3v1 handicap or co-op-vs-bots is allowed — the lobby shows the split, never blocks it), and **friendly fire is off** (allies' shots pass through each other, and turrets and auto-aim ignore allies).

**N facilities are placed around a central asteroid field**, sized to the non-closed slot count. Each player spawns in a ship orbiting their home station with a small stock of **starting ore** (enough to make one meaningful opening choice: an early turret, a head start on upgrades, or banked safety) and **10 seconds of spawn protection** on ship and reactor, so no rush can end a match before anyone has flown. A match countdown ("RUSH!") starts the game. **Bots fill the slots the host set to `bot`, and only those** *(amended 2026-08-07 — this sentence used to read "open slots that no human takes are filled by AI bots", which is the collapse the amendment above undoes)*; before the match, **the host picks each bot's CHARACTER — Rusty, Bolt, Foreman, Patch, Sable, Vulture or Warden (§2.9) — and that character's difficulty is *shown* beside its name, not chosen** (amended 2026-08-07), along with (in Teams) its side. In the lobby, every player also picks a **ship class** (section 2.11) and gets a unique **player color** from the 8-color roster — the color marks their ship trim, weapon fire, shield tint, station beacon ring, and HP bar. In Teams, per-player identity colors are kept and the side is shown as an added indicator (nameplate underline / shared beacon-ring motif), so a single ship stays individually legible on a chaotic screen.

**The bot control is the CHARACTER; the difficulty is a read-out (amended 2026-08-07).** This document used to say the host "picks each bot's difficulty," and the lobby resolved a character from the tier. Two things were wrong with that. It was hard to reason about — *"this bot personality thing makes no sense… its difficult to understand and difficult to balance a team with it"* — and, worse, the resolved character never reached the match, so a host who set every enemy to Hard met a mixed field: *"i chose HARD for all enemies but they were at other difficulties than i selected."* The fix is not to carry the difficulty through more carefully. **It is to make the character the only per-seat control there is**, which does not repair the mismatch so much as delete the possibility of one: there is no second setting left that can disagree with the cast. Three rules follow, and they are design rather than implementation:

- **The tier is shown, never chosen.** Each bot row states its character's difficulty beside the name, as information. §2.9 fixes each character's tier and this document is where that table lives; a lobby that let the two be set apart would be a lobby that can lie about the match it is building.
- **Repeats are allowed.** There are 8 slots and 7 characters, and only 3 of them are Hard, so a full house needs at least one repeat — and a *balanced 4v4* of Hard bots needs a fourth Hard bot that does not exist. Forbidding duplicates would make that arrangement impossible, so the roster seats whoever the host names, twice if asked. A repeated character is told apart by **numbering the name** (`Warden 1`, `Warden 2`) and only when it actually repeats; the slot's identity colour and its `P1`…`P8` decal already separate them on the field (§5.2, style-guide §3 rule 3). **The cast is not extended to dodge this** — §2.9's seven are the cast.
- **Every bot row carries a `?`** that opens that character's codex dossier, and it opens on a **tap** on every device. A hover-only tooltip is a desktop-only feature and the parity principle (§2.4) has no cell for one; the desktop hover and the touch long-press that reach the same dossier are shortcuts to a control that is now advertised, not the affordance itself.
...
**Where the arena is chosen *(amended 2026-08-07)*.** On **MAP SELECT**, a screen opened from the lobby, with the lobby keeping one card — the arena this room is on — as a control that opens it. Same developer report as §2.11's split, same reasoning, and the same non-change underneath: the registry, its count, The Ring as the default and the resource-fairness invariant below are untouched, the pick is still the host's and still locked at RUSH!, and a guest still *reads* the board they are about to fly (the screen opens for them read-only; a control that refuses to open reads as broken).
...
### 2.2 What the player sees *(amended 2026-07-27 — damage-ring grammar; boost/ping cut; amended 2026-08-05 — the side label over every hull; amended 2026-08-07 — station health is always visible, sensor range retired; amended 2026-08-07 — the alarm sounds ONCE per engagement and only for your side's station, and the arrow carries the duration; see `docs/design-amendments.md`)*
...
**Station health is always visible** *(amended 2026-08-07 — developer report a0-05, "you can see other stations healths only when you are near, it should always show the health regardless of proximity or else it looks like a glitch"; this replaces the previous rule, "enemy station health is scouted, not broadcast", which is withdrawn).* Every station's damage ring reads true at every range, whoever owns it — flying toward a home or away from it changes nothing about what its ring says. **The retracted rule was not merely a design preference, it was a display that lied.** Outside sensor range the ring was not drawn at all, and the owner-colour beacon ring underneath it (§5.4, always visible) meant the player saw exactly what a healthy station looks like: no red. A quarter-dead home and an untouched one were the same picture, and the player had no way to tell which they were looking at. A display whose unknown state is indistinguishable from its healthy state does not withhold information — it asserts a false one. Scouting is still how you learn *where* things are and what is happening right now: the minimap is fog-of-war until you build a radar satellite (§2.6), and a ring on the far side of the map is a ring you are not looking at. What is no longer earned is the ring itself. **What did not change:** ship hull bars are still an on-screen read, a rival's ore, bank and upgrade tiers are still drawn for nobody at any range, and enemy health never appears as a **HUD bar** — the top-right panel is your own station's, and the ring on the station is the whole grammar. The sensor-range constant that enforced the old rule is retired (§2.8), and bots read station health on exactly the terms a human does (§2.9).

**The damage-ring grammar (amended).** Every health ring — the reactor's and each shield layer's, yours and any enemy's — reads by one rule: a **whole ring in the owner's color is the health that remains**, and a **threat-red segment fills it clockwise from twelve o'clock, proportional to the health *lost*.** A fully red ring is death, exactly. Because the reactor and its shields share the primitive, a besieged station reads outermost-first — **shields redden and die before the reactor begins to fill** — and red is *only ever damage*, never the station, so an enemy home still reads in its owner's color while it burns. (This corrects an earlier build where the ring read backwards — a shrinking red arc for HP remaining.)
...
Bots are hand-coded behavior trees running the same action interface as human input — no LLM calls at runtime. They fill the lobby's **open slots** and are assigned a side in Teams (§2.1), where they **respect allegiance** — a bot never targets an ally, its shots pass through allies, and its turrets ignore them. Because combat is a travel-time projectile now (§2.3), bots aim on an **intercept course**, leading a moving target by its last-seen velocity and their own muzzle speed; a still target collapses the lead to a straight shot, and per-tier aim jitter means a Hard bot leads well while an Easy bot leads badly — the difficulty ladder is intact. Bots are also **fog-honest**: they perceive only what a human in their cockpit could — the same visual range, the same hidden enemy cargo and bank, and the same station health, which since 2026-08-07 means *both* of them can read a damage ring on any station on their screen (§2.2, amended; a0-05). **The invariant is symmetry, not blindness.** When the sensor-range gate on station HP was retired for the player it was retired for the bot in the same pass, because leaving it in place would have handicapped every bot and quietly moved the whole difficulty ladder; equally, a bot must not read the health of a home four thousand units behind it, which no human can see either. So a Hard bot that knows you're wounded knows it because it looked at you. Difficulty changes visible competence, not cheats:
...
**And since 2026-08-07 that sentence is the literal interface, not an aspiration** *(amended; see §2.1 and `docs/design-amendments.md`)*. It used to describe what the seven *are* while the lobby asked the host for a tier and picked a name from it — the character was flavour laid over the real setting. The lobby now asks for the character by name and reads the tier off this list, so **this table is the control surface**: the seven rows above are what a host chooses between, and Easy/Medium/Hard are what those choices are worth. Nothing about how a bot plays changed with it — the trees, the tuning, the weights, the hulls, the fog honesty and the seeded determinism are all exactly what they were. What changed is who you choose and what you are told about them.
...
### 2.11 Ship classes *(amended 2026-07-27 — "one weapon, one stat"; amended 2026-08-05 — the lobby's hull tiles now SHOW this table, as pips and numbers; **amended 2026-08-07 — the four hull tiles are their own SCREEN, opened from the lobby; the stats rule is unchanged**; see `docs/design-amendments.md`)*
...
**Where the four tiles live *(amended 2026-08-07)*.** They are a **screen of their own — SHIP SELECT — opened from the lobby**, and the lobby keeps **one** card: the hull you have chosen, as a control that opens that screen. The developer, over a screenshot of the live lobby: *"in the lobby page select ship and select map need to open different pages, we should only show 1 ship and 1 map in lobby because it's too cluttered now."* The hull is still chosen in the lobby flow and still locked at RUSH!; what moved is which of the lobby's screens the four are compared on. **Nothing in the stats rule below changes** — and the split is what finally lets it be kept in full, because on a phone the four tiles shared the roster's column and dropped their role blurb on every handset in QA's matrix.
...
### 4.2 Multiplayer: in core scope *(amended 2026-07-27 — no-pause-online, room config advertisement; **amended 2026-08-07 — bots fill only `bot` slots, and a room whose only human is the host boots locally and is released**; see `docs/design-amendments.md` and `docs/variable-slots-plan.md`)*

Online multiplayer is in core scope. A small **authoritative Node.js match server** over WebSockets holds all simulation authority: clients send input ticks, the server runs the one true sim and broadcasts state, clients interpolate. Rooms are created from the lobby with a shareable code; **bots fill the slots the host set to `bot`, server-side, so a classroom match of a few humans is still as full an arena as the host asked for** *(amended 2026-08-07 — this used to read "bots fill open slots", which auto-seated an AI in every seat nobody took; §2.1's amendment is the rule, and the server mirrors it rather than inventing a second one)*.

**A room with no other humans in it plays locally (amended 2026-08-07).** At RUSH!, if the only human in the room is the host, the match is one person against some bots — and that needs no server. It boots on `LocalLoopback` exactly as an offline match does, from the same seed, arena and cast, so it is the *same match* by the determinism rule (§4.8); the room is released at that moment, freeing the code and the seat the fleet was holding. The decision is taken **at RUSH!, never at room creation** — a host may open a room, wait, and have somebody join, and the question is only answerable when the match actually starts. The player is told, once and quietly, that this happened: they pressed a button that said online. A locally-booted match cannot be rejoined by room code, and nothing advertises that it can — the room is gone.
...
A home **station** is a mining installation staking a claimed **planetoid** — the round body is randomised per player from four variants so no two home claims look identical. Oceans are steel-blue and continents patina-green, which keeps the planetoid inside the Cold Vacuum palette instead of importing a second one. The **reactor** stays signal yellow — it is the win condition, so it obeys the yellow rule. Ownership shows as a beacon ring in the player's colour, always visible; health shows as a damage ring, **also always visible** *(amended 2026-08-07, §2.2 — it used to require sensor range, and out of range the beacon ring alone made a wounded home read as untouched)*. **The ring grammar (amended, §2.2):** a whole ring in the owner's colour is the health remaining, and threat-red *fills* it clockwise from twelve o'clock as HP is lost — a fully red ring is death; the reactor and its shields share the primitive, so shields redden and die before the reactor begins to fill, and red is only ever the damage, never the station. *(Lore-pivot note: the existing round-body art reads as the claimed planetoid the station sits on; whether facilities gain additional industrial dressing — rigs, gantries, docking arms — beyond that body is an Art follow-up flagged in the lore changelog, not a mechanic and not this pass's job.)*

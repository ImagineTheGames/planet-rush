...
import type { MatchState, MiningStation, Ship, World } from './state';
...
describe('win/loss (GDD §1)', () => {
  it('the last surviving core wins the match', () => {
    const world = makeWorld({
      ...
      stations: [
        makeStation({ id: 0, owner: 0, pos: at(-500, 0) }),
        makeStation({ id: 1, owner: 1, pos: at(0, 0) }),
        makeStation({ id: 2, owner: 2, pos: at(500, 0) }),
      ],
    });
...
describe('elimination and the wreck (GDD §2.7)', () => {
  function siegeWorld() {
    ...
    const world = makeWorld({
      ...
      stations: [
        makeStation({ id: 0, owner: 0, pos: at(-900, 0) }),
        makeStation({
          id: 1,
          owner: 1,
          pos: at(0, 0),
    ...
    return { world, victim, wreck: world.stations[1]! };
  }

  it('a destroyed core wrecks the station and takes its owner out of the match', () => {
  ...
  it('a wreck cannot be re-eliminated, however many shots land on it', () => {
    ...
    damageStation(world, wreck, 5);
    expect(damageStation(world, wreck, 50)).toBe(false);
    step(world, []);
...
describe('asteroid waves (GDD §2.3)', () => {
  it('every wave scatters strictly closer to the map centre than the last', () => {
    ...
    for (let n = 1; n <= WAVE_COUNT; n++) {
      ...
      // sit out by the stations and were placed at construction, not on the
...
describe('collapse (GDD §2.3)', () => {
  ...
  it('stops shield regeneration', () => {
    ...
    expect(live.station.shields[0]!.hp).toBeCloseTo(10 + SHIELD.regenPerSecond, 6);

    const { world, station } = endgame();
    step(world, []); // the tick collapse opens on
    const hp = station.shields[0]!.hp;
    for (let t = 0; t < 300; t++) step(world, []);
    expect(station.shields[0]!.hp).toBe(hp);
  });
  ...
  it('shuts repair off at collapse — a purchase is refused, entropy only takes', () => {
    ...
    const live = endgame({ wavesSpawned: 0 });
    const liveHp0 = live.station.coreHp;
    const liveBank0 = live.ship.banked;
    expect(placeOrder(live.world, live.ship, 'repair')).toBe('ok');
    expect(live.station.coreHp).toBe(liveHp0 + REPAIR_HP_PER_ORE);
    ...
    const { world, ship, station } = endgame();
    ...
    expect(station.coreHp).toBeCloseTo(coreHp - COLLAPSE_CORE_DECAY * steps * TICK_DT, 6);
    expect(station.coreHp).toBeLessThan(coreHp);

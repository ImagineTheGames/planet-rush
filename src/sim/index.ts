 ...
 * `Shield`/`Projectile`. Building is driven entirely through the action stream
 * (`buildOrder`), so a caller never has to reach into `./buildings` to play the
 * game — the exports there are for the renderer, the bots, and the tests
 * (caps, costs, docking range, mount geometry). Every tuning value lives in
 * `./constants` (GDD §2.8, TUNABLE — QA owns it from day 2).
 */

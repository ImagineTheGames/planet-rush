...
describe('the screen-edge arrow and the readouts it rides past (a0-116)', () => {
   ...
   * groups that carry a word or a number the player reads, which is the set
   * `Hud.arrowKeepOut` hands the live arrow and the set a0-115's
   * `HUD_READOUT_IDS` names. Each rect comes from the element's OWN geometry
   * function, so this is the rect that is drawn and not a restatement of it.
   *
  ...
  it('the home arrow never lands on a readout', () => {
    for (const { name, vp, isTouch } of PROFILES) {
      ...
      for (const bearing of BEARINGS) {
        ...
        //    keeps ARROW_READOUT_PAD of air from it — two runs of ink that end
        ...
        for (const { id, r } of ro) {
          ...
          expect(
            clearAir(drawn, r),
            `${at}: the arrow ${fmt(drawn)} is inside "${id}"'s ${ARROW_READOUT_PAD}px of air`,
          ).toBeGreaterThanOrEqual(ARROW_READOUT_PAD - 1e-9);
        }
  ...
  it('the readouts never cover the ship, which is why the ray always has somewhere to go', () => {
    ...
    for (const { name, vp, isTouch } of PROFILES) {
      ...
      for (const { id, r } of readouts(vp, isTouch)) {
        const covers =
          centre.x > r.x - ARROW_READOUT_PAD &&
          centre.x < r.x + r.width + ARROW_READOUT_PAD &&
          centre.y > r.y - ARROW_READOUT_PAD &&
          centre.y < r.y + r.height + ARROW_READOUT_PAD;
  ...
  it('a0-111s own frame: the arrow stood on WAVE 1/5, and now stands under it', () => {
    ...
    expect(now.y).toBeCloseTo(clock.y + clock.height + ARROW_READOUT_PAD, 6);

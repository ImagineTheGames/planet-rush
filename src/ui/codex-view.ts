...
export class CodexView extends Container {
  ...
  update(model: CodexModel): void {
    ...
    // The screen's own ground, drawn from the beams outward so the beams'
    // translucent fill has something to sit on — unless the shell has already put
    // the real void behind it (a0-79; see `setVoidBehind`).
    this.backdrop.clear();
    if (!this.voidBehind) {
      this.backdrop

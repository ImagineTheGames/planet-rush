...
async function boot(): Promise<void> {
  ...
  //
  // The hull is the one picked in the lobby (`chosenShipClass`), not a hardcoded
  // default — REMATCH reuses it, since "straight to a fresh match" (GDD §2.7) never
  // re-opens the lobby, so a rematch keeps the ship the player chose.

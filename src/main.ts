...
import {
  ...
  mainMenuLayout,
  MAIN_MENU_ITEMS,
  // The ONLINE front door (M3/online step 3): the room-code screen the menu's
  // ONLINE button opens — CREATE a room or JOIN one by code. The transport that
  // opens on a successful allocate is m9-09's; this file only wires the screen and
  // turns the allocator's refusals into the right words.
  LobbyEntryView,
  createEntry,
  entryModel,
  entryFailed,
...
import type { ResolveFailure, OnlineSession, ResolvedConnection, AllocatorClientConfig } from './net';
...
async function boot(): Promise<void> {
  ...
  // The front door tells `boot()` WHICH game to run: an offline `LocalLoopback`
  // (PLAY / PLAY SOLO, and the ?debug=1 harness boot) or a live online session a
  // CREATE / JOIN opened and started (M10). Online, `session.world` already exists
  // — the predicted arena the server started — so the lobby below is skipped and
  // the same render/input loop runs against it (GDD §4.2).
  ...
  if (onlineSession) LOCAL_PLAYER = onlineSession.you;
  ...
  //     personalities visible), the four hull tiles, the four ARENA cards, and
  //     RUSH!. The hull you pick here is the hull you fly and the arena you pick is
  //     the arena the sim builds — both are threaded into `bootOfflineMatch` below,
  //     so the world IS the choice, not a hardcoded default (the M4 dark-matter
  //     bug: the lobby merged in #39 and PLAY never reached it).
  //
  ...
  //     not seats a second player can join. The arena picker used to be its own
  //     step on the PLAY flow (m8-02); the p2 field rule moved it into this one
  //     pre-match room — pick hull AND map here, then RUSH!.
  //
  ...
  //     and map, so the frozen goldens and every existing harness keep their
  //     immediate boot on the default `octagon` board.
  // Online the server owns the roster and the arena, and the match has already
  // started — there is no pre-match lobby to run, so `chosen` falls to the
  // persisted-or-default values (used only for the local nameplate; the hull and
  // world come off the server's own `matchStart`).
  const lobby = flags.debug || onlineSession ? null : openLobby(app, platform, frontCtx);
 ...
 *
 *  - `offline` — PLAY / PLAY SOLO: `boot()` builds the `LocalLoopback` match, the
 *    shipping product, exactly as before (the default when there is no menu).
 *  - `online` — a CREATE / JOIN that reached the allocator, opened a real socket
 *    (`createOnlineSession`, the proven client-sim the integration tests drive),
 *    and the match started: `session.world` is already the predicted arena, so
 *    `boot()` renders it through the same loop, feeding `sendInput` in place of the
 *    offline clock (GDD §4.2).
 */
type LaunchPlan =
  | { readonly kind: 'offline' }
  | { readonly kind: 'online'; readonly session: OnlineSession };
...
interface MainMenuHandle {
  /** Resolves the moment PLAY (offline) or a started online match (CREATE / JOIN)
   *  hands off — `boot()` builds/renders the world only after, and the
   *  {@link LaunchPlan} tells it which. */
...
interface MainMenuSeam {
  ...
  play(): void;
  /** Open the ONLINE front door (M3/online) — the same as tapping ONLINE. */
  online(): void;
}

/**
 * The read-only `window.__onlineMenu` seam. Lets the live-stage suite drive the
 * ONLINE front door — CREATE / JOIN, the code keypad, and the three distinct
...
interface OnlineSeam {
  /** True while the ONLINE screen owns the display. */
  visible: boolean;
  ...
  submit(): void;
  back(): void;
  /** RUSH!, online — the room creator starts the match once a peer has joined.
   *  A no-op for a joiner (and until a room is created). */
  start(): void;
}
...
function openMainMenu(
  ...
  let played = false;
  // The ONLINE front door's own state (M3/online). `entry` is the CREATE/JOIN
  // model; `onlineResolved` holds the allocator's minted code once a CREATE
  // succeeds (never client-guessed — M3 rule). One region at launch, so the
  // picker stays suppressed by config (`regionPickerVisible === false`).
  ...
  const seam: MainMenuSeam = {
    ...
    play: (): void => play(),
    online: (): void => openOnline(),
  };

  // The ONLINE front door's own read-only seam. Its methods drive the same pure
  // transitions a tap would, so the live-stage suite can walk CREATE / JOIN / a
  // refusal without pixel math (tests/live-stage/online-flow.spec.ts).
  ...
  // match starts and `boot()` takes it over (`beginOnline`). Null on the offline
  // path, which never enters this file's socket code at all.
  let onlineSession: OnlineSession | null = null;
  // True when THIS client created the room: only the creator may RUSH! (the
  // server enforces it too — `server/room.ts` `startMatch`), so only the creator's
  // start gesture is live.
  let onlineHost = false;
  // One-shot latch so a duplicate `matchStart` (e.g. a reclaim) never hands off
  // twice.
  ...
  /** Mirror the entry model onto the `__onlineMenu` seam after every change. */
  ...
  /** Open the room-code front door — the ONLINE button (and its seam). */
  ...
  /** Back out of the front door to the main menu. */
  ...
  /** Tap (or seam-drive) a door on the ONLINE home screen. SOLO is the offline
   *  escape hatch — it needs no server, so it just starts the offline game (PLAY).
   *  CREATE / JOIN go through the allocator. */
   ...
   *
   * `matchStart` is the handoff: the session builds the predicted arena from the
   * server's own seed and roster, and `beginOnline` renders it through the exact
   * loop the offline match uses.
   */
  function connectMatch(
    ...
    session.observe((message) => {
      if (message.type === 'matchStart') beginOnline(session);
      // A refused join is terminal — the transport has already stopped and will
      // NOT redial (`websocket-transport` M10), so if we drop this frame the
      // player waits on "connecting" forever. Surface it on the front door
      // instead, with the reason and a one-tap retry (a *fresh* allocate).
      else if (message.type === 'joinError') rejectOnline(message.reason);
    });
    // A JOINER waits here for the host to RUSH!; the host may start now.
  }

  /**
   * The match server refused this client's join (M10 machine-pin lottery, or an
   * expired/forged ticket): leave the "connecting" screen for the front door's
   * graceful error, classifying the server's reason into the same words an
   * allocator refusal earns (`resolveFailureForJoinError`) and keeping the typed
   * code so RETRY is a single fresh allocate — never the eternal spinner a dropped
   * `joinError` used to leave. Ignored once the match has begun (no in-flight join
   * to refuse) or if the player has already left the front door.
   */
  function rejectOnline(reason: string): void {
    if (onlineStarted || screen !== 'online') return;
    onlineSession?.close();
    failOnline(resolveFailureForJoinError(reason));
  }

  /** RUSH!, online: the room creator starts the match. A no-op for a joiner or
   *  before a room exists — the server also refuses a non-creator's start
   *  (`server/room.ts`). Driven by the host's start gesture and the online seam. */
  function startOnline(): void {
    if (!onlineSession || !onlineHost || onlineResolved === null) return;
    onlineSession.startMatch();
  }

  /** The match started on the server: the session's predicted world exists, so
   *  tear the menu down and hand `boot()` the live session to render through the
   *  shared loop. Latched so a reclaim's second `matchStart` never re-hands-off. */
  function beginOnline(session: OnlineSession): void {
    if (onlineStarted) return;
    onlineStarted = true;
    played = true; // close the offline PLAY door — this client is committed online
    ctx.enterImmersive();
    teardown();
    seam.visible = false;
    resolvePlay({ kind: 'online', session });
  }

  /** Show an allocator refusal on the front door, keeping the typed code so a
   *  retry is one tap (`entryFailed`), and speaking the reason's own words. */
  ...
  function onPointerDown(e: PointerEvent): void {
    ...
    if (screen === 'menu') {
      ...
      // Host, room created, tap off the controls: RUSH! (the touch twin of the
      // Enter/Space start — a joiner's tap here does nothing but wait).
      else if (onlineHost && onlineResolved !== null) startOnline();
  ...
  function onKeyDown(e: KeyboardEvent): void {
    if (screen === 'online') {
      // A room is created and this client is the host: Enter / Space is RUSH! —
      // the same start gesture as the menu's PLAY, now that the peer has the code.
      if (onlineHost && onlineResolved !== null && (e.code === 'Enter' || e.code === 'Space')) {
        startOnline();
        return;
      }
 ...
 * The local player is seat 0 and the host, so hull selection, ARENA selection,
 * per-seat difficulty cycling, and RUSH! are all live.
 *
 * The arena picker lives here now (p2 field rule): one pre-match room where you
 * pick your hull AND your map, then RUSH! — no separate picker step on the PLAY
 * flow. Both picks are persisted (same storage seam as the fire mode) and both are
 * resolved to `boot()` at RUSH!.
 *
 ...
 * and the arena the sim ended up building.
 */
function openLobby(
  app: Application,
  platform: ReturnType<typeof createBrowserPlatform>,
   ...
   *  after RUSH!, where {@link selectMap} locks the arena with the hull. */
  function selectMapAt(index: number): void {
    if (resolved) return;
    state = selectMap(state, mapIdAt(index));

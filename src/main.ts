...
import { OFFLINE_ROOM, allocatorUrlFromEnv, allocateRoom, joinRoom } from './net';
import type { ResolveFailure } from './net';
...
function openMainMenu(
   ...
   * right words. The transport that opens on a *successful* resolve — and the
   * online lobby it leads to — is m9-09's; this milestone owns the screen, so a
   * success here records the minted code and stops (m9-09 fills the handoff).
   * Every *refusal* is fully wired: the three failures the allocator distinguishes
   * become three different messages, and with no allocator configured (the shipped
   * offline build) the honest answer is the graceful "can't reach the servers".
   */
  async function startResolve(door: 'create' | 'join', room: string): Promise<void> {
    ...
    // Success: the code is the allocator's, used verbatim (M3 rule). Opening the
    // socket + the online lobby is m9-09's seam — recorded here for it to take.

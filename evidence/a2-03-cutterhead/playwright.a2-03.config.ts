/**
 * evidence/a2-03-cutterhead/playwright.a2-03.config.ts — the golden suite, on a port
 * this lane owns. OWNER: Art Agent (brief a2-03).
 *
 ...
 * other lanes, and it bit: a3-01 re-shot a build-wheel golden and got a
 * byte-identical frame back because another lane's `vite preview` held 4173 and
 * Playwright happily reused it — the frame was a different branch's build. A golden
 * shot against someone else's bundle is worse than no golden.
 *
 ...
 * the served bundle back and asserts the Cutterhead is in it before any shot
 * is taken, so "wrong build" can never again be a silent pass.
 *
 ...
 *
 *   node evidence/a2-03-cutterhead/verify-served-build.mjs      # after the server is up
 *   npx playwright test --config evidence/a2-03-cutterhead/playwright.a2-03.config.ts \
...
const PORT = Number(process.env.A2_03_PREVIEW_PORT ?? 4291);

/**
 * evidence/a0-127-three-changes-with-eyes/plates.mjs — compose the manifest's plates.
 * OWNER: QA Manager (a0-118).
 *
 ...
 *
 * Inherited from a0-118 (a0-99's, and a0-96's before that) unchanged except for
 * its paths, deliberately: two briefs whose plates are composed by the same script
 * are two briefs whose plates can be compared without asking whether the
 * difference is in the evidence or in the layout.
 *
 * a0-127 adds one rule of its own, because one of its verdicts is about SIZE: a
 * plate whose `width` equals its frame's own pixel width renders that frame 1:1,
 * and the MAP SELECT plate is built that way on purpose. Where a plate DOES scale
 * a frame down (the whole-board frames are 3300px wide), the caption says so and
 * names the full-size file in `shots/`.
 *
...
function html(plate) {
  const blocks = plate.blocks
    .map((b) => {
      // `crops/…` paths resolve against the brief dir, everything else against shots/.
      const src = `file://${b.img.startsWith('crops/') ? join(HERE, b.img) : join(SHOTS, b.img)}`;

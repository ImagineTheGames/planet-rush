...
## Found on the way past: the golden gate cannot see this sky

The re-baseline moved eleven images. Seven moved on the first
`--update-snapshots` run; four — `desktop-title`, `desktop-settings`,
`desktop-doors`, `phone-landscape-doors` — passed against their old, sky-less
baselines. Those four are exactly the menu screens whose **geometry** did not
change; the seven that moved all had a column or a band move as well.

The sky is not missing on them. `evidence/a0-79-menus/sky-check.mjs` shoots each
screen twice from the shipped bundle, `?sky=1` vs `?sky=0`, and counts:

| screen | pixels differing |
| --- | --- |
| desktop 1280×800 menu | **21.00%** |
| desktop 1280×800 doors | **20.93%** |
| phone 844×390 dpr3 menu | **15.24%** |
| phone 844×390 dpr3 doors | **17.29%** |

All far past the goldens' `maxDiffPixelRatio` of 1%. And deleting
`desktop-title-desktop-linux.png` and re-running that one test wrote a 395 KB
starry frame where the old baseline was 39 KB and flat — the capture was right
all along.

The likely cause is Playwright's per-pixel `threshold` (default 0.2 normalised
YIQ): this void is Floor `#070910` where the panel was Vacuum `#0d1015`, plus a
low-alpha wash and small stars. It changes nearly every pixel *by a little*, and
a little on a near-black frame is under the per-pixel threshold, so under 1% of
pixels count as different at all.

**A blind spot in the gate, not in this branch** — a change that alters a whole
frame gently is invisible to these baselines. It is QA's file and QA's number, so
nothing here touches it; the four stale baselines were deleted and regenerated by
hand (a *missing* snapshot is always written, the one path that cannot be
silently skipped), so all eleven committed images depict what ships.

/**
 * evidence/a0-88-minimap/playwright.config.ts — OWNER: UI Engineer (a0-88).
 *
 * A private config, so this capture never runs inside QA's suite and never
 * touches `tests/mobile/`. The two rules a0-62 set and every capture since has
 * kept:
 *
 ...
 *
 * No viewport is pinned here: every frame is shot at 798×384 dpr 3, the
 * developer's own phone viewport, set per-context in `shots.spec.ts`.
 *
 * `A0_88_REUSE=1` skips the build and attaches to a server this run did not
...
const PORT = Number(process.env.PREVIEW_PORT ?? 4288);
...
export default defineConfig({
  ...
  webServer: {
    ...
    url: URL,
    reuseExistingServer: Boolean(process.env.A0_88_REUSE),
    timeout: 900_000,
  },
});

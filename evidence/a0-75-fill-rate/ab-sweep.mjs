...
/** Frozen only. The live match adds bot AI and entity churn, which is real cost
 *  but is not the axis under test and adds variance the A/B cannot absorb; the
 *  frozen scene pins the sim at the golden tick so the ONLY difference between
 *  the two columns is the render path. */
const QUERY = '?debug=1&freeze=1';
...
async function main() {
  ...
  for (const [i, vp] of VIEWPORTS.entries()) {
    ...
    rows.push(row);
    console.log(
      `${vp.name.padEnd(10)} ${row.megapixels.toFixed(2).padStart(5)} Mpx  ` +
  ...
  writeFileSync(
    new URL('./ab-sweep.json', import.meta.url),
    `${JSON.stringify({ map: MAP, frames: FRAMES, settle: SETTLE, query: QUERY, rows }, null, 2)}\n`,
  );
  console.log('\nwrote ab-sweep.json');
}

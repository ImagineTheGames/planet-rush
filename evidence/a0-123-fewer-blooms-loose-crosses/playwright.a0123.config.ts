/**
 * evidence/a0-123-fewer-blooms-loose-crosses/playwright.a0123.config.ts — a0-123 only.
 *
 ...
 *
 * **a0-123 inherits this file wholesale, and the reasons with it.** It is a copy
 * rather than an import because the port has to differ — two lanes re-baselining
 * at once on one box is the very collision the paragraph above describes — and
 * because an evidence config is a record of how ONE run was made. See
 * `../a0-45-star-temperature-colour/goldens-rebaseline.md` for the method; the
 * tolerance trap it documents applies here unchanged and is why this brief's
 * run also sets `maxDiffPixelRatio: 0` for the duration and does not commit it.
 *
